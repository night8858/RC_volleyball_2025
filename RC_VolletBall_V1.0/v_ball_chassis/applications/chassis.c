...
void chasis_Task(void const *argument)
{
    ...
    while (1)
    {
        ...
        if (rc_ctrl.rc.s[0] == 1 && rc_ctrl.rc.s[1] == 1)
        {
            ...
            // vofa的测试代码
            uart_dma_printf(&huart1,"%4.3f ,%d ,%4.3f ,%4.3f\n",
            motor_control.M6020_M1.serial_angle , 
            motor_control.M6020_M1.given_current ,
...
static void motor_control_loop(motor_control_t *control_loop)
{
    ...
    //CAN_cmd_6020(motor_control.M6020_M1.given_current, 0, 0, 0);
...
static void motor_feedback_update(motor_control_t *feedback_update)
{
    ...
    process_motor_encoder_to_serial(&feedback_update->M6020_M1);
    ...
    process_motor_encoder_to_serial(&feedback_update->M6020_M2);
    ...
    process_motor_encoder_to_serial(&feedback_update->M6020_M3);
    ...
    process_motor_encoder_to_serial(&feedback_update->M6020_M4);
    // 数据更新,各个航向电机的角度数据
...
static void movement_calc(void)
{
    ...
    process_encoder_to_serial(&chassis_float);
    //chassis_float.serial_angle = 5.0f;
    ...
    {
        motor_control.M6020_M1.relative_angle_set = chassis_float.serial_angle + 6144 * MOTOR_ECD_TO_RAD;
        motor_control.M6020_M2.relative_angle_set = chassis_float.serial_angle + 3413 * MOTOR_ECD_TO_RAD;
        motor_control.M6020_M3.relative_angle_set = chassis_float.serial_angle + 7509 * MOTOR_ECD_TO_RAD;
        motor_control.M6020_M4.relative_angle_set = chassis_float.serial_angle + 2048 * MOTOR_ECD_TO_RAD;
    }
...
void process_motor_encoder_to_serial(motor_6020_t *motor)
{	
	//处理成连续的码盘值
	if(motor->relative_angle - motor->last_angle > 5.0f) 
		--motor->circle_num;
	else if(motor->relative_angle - motor->last_angle < -5.0f) 
		++motor->circle_num;
	motor->serial_angle = motor->relative_angle + motor->circle_num * 2 * PI;
		//码盘值转换成角度
	//motor->position_angle = motor->serial_angle / 8192.0f /180.0f * PI;
	motor->last_angle =  motor->relative_angle;	
}

void process_encoder_to_serial(chassis_move_date *motor)
{	
	
	//处理成连续的码盘值
	if(motor->angle - motor->last_angle > 5.0f) 
		--motor->circle_num;
	else if(motor->angle - motor->last_angle < -5.0f) 
		++motor->circle_num;
	motor->serial_angle = motor->angle + motor->circle_num * 2 * PI;
		//码盘值转换成角度
	//motor->position_angle = motor->serial_angle / 8192.0f /180.0f * PI;
