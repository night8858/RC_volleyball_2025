...
void ball_track_calc(void)
{
    ...
    float hit_distance = BALL_TRACK_TARGET_DEEPTH + BALL_TRACK_HIT_DOWN * ball_track_target.hit_num;
...
void process_motor_encoder_to_serial(angle_t *motor)
{	
    
	motor->angle = ball_track_target.angle;
	//处理成连续的码盘值
	if(motor->angle - motor->last_angle > 5.0f) 
		--motor->circle_num;
	else if(motor->angle - motor->last_angle < -5.0f) 
		++motor->circle_num;
	motor->serial_angle = motor->angle + motor->circle_num * PI;
		//码盘值转换成角度	
